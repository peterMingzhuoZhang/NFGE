...
void ShowGraph(NavGraph & graph, float nodeRadius)
{
	for (size_t i = 0; i < graph.GetNodeCount(); i++)
	{
		...
		DrawScreenCircle(currentNode.position, nodeRadius, Graphics::Colors::LightBlue);
...
void ShowGraph(NavGraph & graph, const std::vector<Tile>& allTile, float nodeRadius)
{
	for (size_t i = 0; i < graph.GetNodeCount(); i++)
	{
		...
		{
			DrawScreenCircle(currentNode.position, nodeRadius, Graphics::Colors::Yellow);
			continue;
		}
		DrawScreenCircle(currentNode.position, nodeRadius, Graphics::Colors::DarkBlue);
...
void ShowGraph(NavGraph & graph, const std::vector<Tile>& allTile, float nodeRadius, const Camera2D & camera)
{
	for (size_t i = 0; i < graph.GetNodeCount(); i++)
	{
		...
		if (!camera.IsOutOfView(inScreenPosition, Vector2()))
		{
			if (allTile[graph.GetNode(i).tileType].isWall)
			{
				DrawScreenCircle(inScreenPosition, nodeRadius, Graphics::Colors::Yellow);
				continue;
			}
			DrawScreenCircle(inScreenPosition, nodeRadius, Graphics::Colors::DarkBlue);
...
void ShowGraph(NavIneractGraph & graph)
{
	...
	{
		MapNode& currentNode = graph.GetNode(i);
		DrawScreenCircle(currentNode.position, graph.GetNode(i).radius, Graphics::Colors::LightBlue);
...
void ShowGraph(NavSpringGraph & graph, const Camera2D & camera)
{
	...
	{
		MapNode& currentNode = graph.GetNode(i);
		DrawScreenCircle(camera.ConvertToScreen(currentNode.position), graph.GetNode(i).radius * camera.GetScale(), Graphics::Colors::LightBlue);
