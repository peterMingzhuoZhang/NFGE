...
void BuildGridBaseGraph(NavGraph & graph, int col, int row, float xGap, float yGap, Vector2 initPosition)
{
	...
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < col; j++)
		{
			...
			float halfGapX = xGap * 0.5f;
			float halfGapY = yGap * 0.5f;

			currentNode.position = Vector2{ initPosition.x + j * xGap + halfGapX, initPosition.y + i * yGap + halfGapY};
			currentNode.rect = NFGE::Math::Rect(
				currentNode.position.x - halfGapX,
				currentNode.position.y - halfGapY,
				currentNode.position.x + halfGapX,
				currentNode.position.y + halfGapY
			);
		}
...
void ShowGraph(NavGraph & graph, float nodeRadius)
{
	for (size_t i = 0; i < graph.GetNodeCount(); i++)
	{
		...
		DrawScreenCircle(currentNode.position, nodeRadius, Colors::LightBlue);
...
void ShowGraph(NavGraph & graph, const std::vector<Tile>& allTile, float nodeRadius)
{
	for (size_t i = 0; i < graph.GetNodeCount(); i++)
	{
		...
		{
			DrawScreenCircle(currentNode.position, nodeRadius, Colors::Yellow);
			continue;
		}
		DrawScreenCircle(currentNode.position, nodeRadius, Colors::DarkBlue);
...
void ShowGraph(NavIneractGraph & graph)
{
	...
	{
		MapNode& currentNode = graph.GetNode(i);
		DrawScreenCircle(currentNode.position, graph.GetNode(i).radius, Colors::LightBlue);
...
void ShowGraph(NavSpringGraph & graph, const Camera2D & camera)
{
	...
	{
		MapNode& currentNode = graph.GetNode(i);
		DrawScreenCircle(camera.ConvertToScreen(currentNode.position), graph.GetNode(i).radius * camera.GetScale(), Colors::LightBlue);
