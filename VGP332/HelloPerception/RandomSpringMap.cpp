...
void RandomSpringMap::Render(const Camera2D& camera)
{
	...
	if (isGraphUpdate)
	{
		if (isGraphPathFinded)
		{
			...
			ShowClosedList(GraphContext, randomGraph, 4.0f, Colors::Red,camera);
			ShowPath(GraphContext, randomGraph, 4.0f, Colors::LightGreen, camera);
		}
	}
	...
	for (int i = 0; i < PathEndPoints.size(); i++)
	{
		...
		DrawScreenCircle(camera.ConvertToScreen(currentNode.position), currentNode.radius * camera.GetScale(), Colors::LightGreen);
	}
