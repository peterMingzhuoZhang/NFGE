...
void TextureGridMap::Open(std::string fileName)
{
	...
	for (int i = 0; i < tileKindCount; ++i)
	{
		...
		allTiles[i].index = i;
		allTiles[i].textureId = NFGE::Graphics::TextureManager::Get()->LoadTexture(allTiles[i].textureName.c_str());
	}
...
void TextureGridMap::Load()
{
	...
	xScale = (graphMaxArea.x / col) / textureSize.x;
	yScale = (graphMaxArea.y / row) / textureSize.y;
...
void TextureGridMap::Render(const Camera2D& camera)
{
	...
	if (isGraphUpdate)
	{
		...
		if (activeNode != -1)
		{
			NFGE::sApp.DrawScreenRect(camera.ConvertToScreen( gridGraph.GetNode(activeNode).rect), Colors::Cyan);
		}
	}
	...
	if (isGraphPathFinded)
	{
		...
		ShowClosedList(GraphContext, gridGraph, 6.0f*  simpleDrawScale, Colors::Red,camera);
		ShowPath(GraphContext, gridGraph, 6.0f*  simpleDrawScale, Colors::LightGreen, camera);
	}

	//int start = startX + (startY * col);
	//int end = endX + (endY * col);
	//DrawScreenCircle(camera.ConvertToScreen(gridGraph.GetNode(start).position), 10.0f*  simpleDrawScale * camera.GetScale(), Color::LightGreen());
	//DrawScreenCircle(camera.ConvertToScreen(gridGraph.GetNode(end).position), 10.0f*  simpleDrawScale * camera.GetScale(), Color::LightGreen());
}

int TextureGridMap::ConvertToMapIndex(const Vector2& position) const
{
	Vector2 relativePosition = position - (initPosition);
...
bool TextureGridMap::IsValidIndex(int index) const
{
	if ((index >= 0) && (index < gridGraph.GetNodeCount()))
		return true;
	else
		return false;
}
