...
//				PathfindingMap: [Baisc Node Structure for Each PathFindMap]
...
#ifndef HELLOPERCEPTION_GRAPHUTIL_HEADER
#define HELLOPERCEPTION_GRAPHUTIL_HEADER
...
struct PolygoneMapNode : public MapNode
{
	size_t worldGraphIndex = AI::InvalidNode;
	size_t inPolygonIndex = AI::InvalidNode;
	PolygoneMapNode() = default;
	PolygoneMapNode(const MapNode& mapNode)
		: MapNode(mapNode)
	{
		neighbors = mapNode.neighbors;
	}
};
...
struct MapInteractNode : public MapNode
{
	...
	void ShowActive()
	{
		DrawScreenCircle(position, radius, Colors::Yellow);
	}
	void ShowActive(const Camera2D& camera )
	{
		DrawScreenCircle(camera.ConvertToScreen(position), radius* camera.GetScale(), Colors::Yellow);
	}
};
...
struct MapSpringNode : public MapInteractNode
{
	...
	void Update(const AI::Graph<MapSpringNode>& graph, Vector2 center, float deltaTime)
	{
		...
		for (int i = 0; i < static_cast<int>(springLengths.size()); ++i)
		{
...
template<typename T>
inline void ShowGraph(T& graph, const std::vector<Tile>& allTile, float nodeRadius, const Camera2D& camera);
...
inline std::list<size_t> GetPath(AI::GraphSearch::Context & context);
...
inline void ShowGraph(T& graph, const std::vector<Tile>& allTile, float nodeRadius, const Camera2D& camera)
{
	for (size_t i = 0; i < graph.GetNodeCount(); i++)
	{
		...
		if (!camera.IsOutOfView(inScreenPosition, Vector2()))
		{
			...
			{
				DrawScreenCircle(inScreenPosition, nodeRadius, Colors::Yellow);
				continue;
			}
			DrawScreenCircle(inScreenPosition, nodeRadius, Colors::DarkBlue);
...
void DrawLineBetweenNeighbor(T& graph, const MapNode& currentNode)
{
	...
	for (size_t j = 0; j < currentNode.neighbors.size(); j++)
	{
		...
		DrawScreenLine(currentNode.position, neighbourNode.position, Colors::Gray);
	}
}
...
void DrawLineBetweenNeighbor(T& graph, const MapNode& currentNode, const Camera2D& camera)
{
	...
	for (size_t j = 0; j < currentNode.neighbors.size(); j++)
	{
		...
		Color lineColor = { Colors::Gray.x, Colors::Gray.y, Colors::Gray.z, 0.2f };
		DrawScreenLine(camera.ConvertToScreen(currentNode.position), camera.ConvertToScreen(neighbourNode.position), lineColor);
	}
