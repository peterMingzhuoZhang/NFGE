#ifndef LACELLECRAFT_GRAPHUTIL_HEADER
#define LACELLECRAFT_GRAPHUTIL_HEADER
...
struct MapInteractNode : public MapNode
{
	...
	void ShowActive()
	{
		DrawScreenCircle(position, radius, Yellow);
	}
	void ShowActive(const Camera2D& camera )
	{
		DrawScreenCircle(camera.ConvertToScreen(position), radius* camera.GetScale(), Yellow);
	}
};
...
template<typename T>
inline void ShowPath(std::list<size_t>& path, T& graph, float nodeRadius, NFGE::Graphics::Color color);
template<typename T>
inline void ShowPath(std::list<size_t>& path, T& graph, float nodeRadius, NFGE::Graphics::Color color, const Camera2D& camera);


template<typename T>
void ShowPath(AI::GraphSearch::Context& context, T& graph, float nodeRadius, NFGE::Graphics::Color color);
template<typename T>
void ShowPath(AI::GraphSearch::Context& context, T& graph, float nodeRadius, NFGE::Graphics::Color color, const Camera2D& camera);

template<typename T>
void ShowClosedList(AI::GraphSearch::Context& context, T& graph, float nodeRadius, NFGE::Graphics::Color color);
template<typename T>
void ShowClosedList(AI::GraphSearch::Context& context, T& graph, float nodeRadius, NFGE::Graphics::Color color, const Camera2D& camera);
...
template<typename T>
inline void ShowPath(AI::GraphSearch::Context & context, T & graph, float nodeRadius, NFGE::Graphics::Color color, const Camera2D & camera)
{
	if (context.found)
	{
		std::list<size_t> path;
...
template<typename T>
inline void ShowClosedList(AI::GraphSearch::Context & context, T & graph, float nodeRadius, NFGE::Graphics::Color color, const Camera2D & camera)
{
	for (auto& nodeIndex : context.closedList)
	{
		MapNode& nodeB = graph.GetNode(nodeIndex);
