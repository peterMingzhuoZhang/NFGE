...
{
	Color theColor = GraphicsSystem::Get()->GetClearColor();
	float ClearColor[4] = { theColor.x, theColor.y, theColor.z, theColor.w }; // RGBA
...
void NFGE::Graphics::RenderTarget::ResumeRender()
{
	auto context = GetContext();
	context->OMSetRenderTargets(1, &mRenderTargetView, mDepthStencilView);
	context->RSSetViewports(1, &mViewport);
...
void NFGE::Graphics::RenderTarget::BindVS(uint32_t slot)
{
...
void NFGE::Graphics::RenderTarget::UnbindVS(uint32_t slot)
{
	static ID3D11ShaderResourceView* dummy = nullptr;
