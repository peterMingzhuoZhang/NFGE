...
namespace
{
	...
	void UpdateTransfrom(Bone* bone, std::vector<PA::TentacleSegment>& armSegments)
	{
		if (bone->parent)
		{
			armSegments[bone->index].boneTranslateMatrix = Matrix4::sTranslation(armSegments[bone->index].mAnitmatePosition) * armSegments[bone->parentIndex].boneTranslateMatrix;
			Matrix4 mat = armSegments[bone->index].boneTranslateMatrix;
			armSegments[bone->index].boneMatrix = Inverse(mat) *  bone->toParentTransform *mat* armSegments[bone->parentIndex].boneMatrix;

		}
		else
		{
			armSegments[bone->index].boneMatrix = bone->toParentTransform;
			armSegments[bone->index].boneTranslateMatrix = Matrix4::sIdentity();
...
void PATentacle::Load(int totalSeg, float baseLength, const char* diffuseMap, const char* nomralMap, const char* specularMap, const char* displaceMentMap)
{
	...
	// - Mesh Initlize -----------------------------------------------------------
	mMesh = NFGE::Graphics::MeshBuilder::CreateTentacle(mTotalSeg, 500, 4, 2.0f, baseLength * mTotalSeg, NFGE::Math::Ease::EaseNone);
...
void PATentacle::Update(float deltaTime)
{
	ControlSegment(deltaTime);
}

void PATentacle::Render(const Camera & camera)
{
	mVertexShader.Bind();
	mPixelShader.Bind();
	...
	PA::TransformData transformData;
...
void PATentacle::DebugUI()
{
	// - Imgui control -------------------------------------------------------------------------------------------------------------------------------
	ImGui::Begin("Control PATentacle");
	...
	for (int i = 0; i < mSegs.size(); i++)
	{
		Vector3 bone_pos_0 = Vector3{ 0.0f,0.0f,0.0f } *mSegs[i].boneTranslateMatrix* mSegs[i].boneMatrix ;
		Vector3 bone_pos_1 = Vector3{ 0.0f,0.0f,0.0f } *Matrix4::sTranslation(mSegs[i].mChildSpwanPoint)* mSegs[i].boneTranslateMatrix*mSegs[i].boneMatrix;
...
void PATentacle::BuildBoneSkelton(int totalBone, float baseLength)
{
...
void PATentacle::ControlSegment(float deltaTime)
{
	...
	{
		for (int i = 1; i < static_cast<int>(mSegs.size()); i++)
		{
...
void PATentacle::UpdateSegmentTransform(int index)
{
	auto& currentToParent = mSegs[index].mBone.toParentTransform;

	currentToParent = NFGE::Math::MatrixRotationQuaternion(mSegs[index].mAnitmateRotation);
