#ifndef HELLOPOSTPROCESSINGADVANCE_POSTPROCESSSPHERER_HEADER
#define HELLOPOSTPROCESSINGADVANCE_POSTPROCESSSPHERER_HEADER
...
struct MySphere
{
	...
	virtual void PostProcess_Render(const Camera& camera)
	{
...
struct MySpherePC : MySphere
{
	...
	virtual void PostProcess_Render(const Camera& camera)
	{
		MySphere::PostProcess_Render(camera);
	}
};
struct MySpherePN : MySphere
{
	...
	void PostProcess_Render(const Camera& camera) override
	{
		mMatrices[0].Identity();
...
struct MySpherePX : MySphere
{
	...
	void PostProcess_Render(const Camera& camera) override
	{
		...
		auto rs = RasterizerStateManager::Get()->GetRasterizerState(IsSkyDome ? "CullFrontSolid" : "Solid");
		rs->Set();
...
struct SphereStandard : MySphere
{
	...
	void PostProcess_Render(const Camera& camera) override
	{
		...
		transformData.viewPositon = camera.GetPosition();
		transformData.viewDir = camera.GetDirection();
		transformData.cameraDepth = camera.GetFarPlane() - camera.GetNearPlane();
		mTransformBuffer.Load(transformData);
		mTransformBuffer.BindVS(0);
